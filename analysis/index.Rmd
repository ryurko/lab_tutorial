...
for creating / maintaining a research website using R Markdown and version control. 
...
host your site. __No prior knowledge, [or even installation](https://jdblischak.github.io/workflowr/articles/wflow-04-how-it-works.html), of Git is necessary to get started!__
...
To create a project with `workflowr`, first access the package:
...
this does NOT need to be the same name / email used for GitHub or GitLab. Since
`workflowr` relies on [`git2r`](https://cran.r-project.org/package=git2r) (which
...
files (`index.Rmd`, `about.Rmd`, `license.Rmd`) were not 'knit' yet. Rather than
manually doing this, `workflowr` naturally provides a function to generate the
appropriate HTML files in the `docs/` folder:

```{r build-demo, echo = TRUE, eval = FALSE}
wflow_build()
```

![](assets/wflow_build.png)
...
In order to actually view the website for the project you need to link it to a 
GitHub/GitLab repository. This will then host the HTML files via their Pages 
feature. You can create the repository from within R (following permission prompts):
...
Once the above steps are complete, then you need to "push" your edits to their
